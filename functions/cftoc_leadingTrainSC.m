function [feas_l, xOpt_l, uOpt_l, JOpt_l] = cftoc_leadingTrainSC(x0, ... 
     xbar, ubar, MODEL, param, slope_,radius_,DPspeed_,maxspeed_)
%% solves CFTOC problem for the leading train
% With soft constraints on:
% - terminal constraint v <= v_DP
%% Inputs
...
if MODEL == midterm
    ...
    Np = param.Np ;
    g = param.g ;   
    K_SC = 1e6; % Move this to params
    
    %% Define Opitimization variables
    ...
    u = sdpvar(1,Np);
    assign(u,ubar);
    % define optimization variable for soft constraint
    eps = sdpvar(1,1); % so far only one on terminal velocity

    %% define objective function
    
    objective = K_SC * abs(eps);  % soft constraint
    ...
    constraints = [constraints x(:,i+1) == train_dynamics_midterm(x(:,i), u(1,i), param,slope_,radius_,DPspeed_,maxspeed_)... %  with estimated values  alternatively: x(:,i+1) == train_dynamics(x(:,i), u(1,i), param) used in paper
        ...
        -1.1 * M * g * param.mumax  <= u(1,i) <= 1.1 * M * g * param.mumax]; 
        %-Pbr <= x(2,i+1) * u(1,i) <= Pdr];
    end
    %constraints  = [constraints 0 <= x(2,Np+1) <= limspeed_(x(1,Np+1),p_sampled ,vOpt_DP)];
    constraints  = [constraints 0 <= x(2,Np+1) <= DPspeed_(xbar(1,Np+1)) - eps];  % SOFT CONSTRAINT
...
elseif MODEL == paper
    ...
    objective =  K_SC * eps;  % soft constraint
