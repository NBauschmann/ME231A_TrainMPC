function [feas, xOpt, uOpt, predErr, x_pred_l, x_pred_f, u_pred_l, u_pred_f] = MPC_MovingBlock(x0_l, x0_f,param,MODEL,...
...
uOpt_f_t = 1*1.8449e+05*ones(1,param.Np) ;
...
%         predErr_f = [];
       ...
       u_pred_l{i} = uOpt_l_t ;
        ...
        [feas_f_t, xOpt_f_t, uOpt_f_t, JOpt_f_t] = cftoc_followingTrainMB(x0_f, ... 
            xbar_f, ubar_f, xOpt_l_t(:,1), MODEL, param, slope_,radius_,DPspeed_,maxspeed_) ;
...
%             predErr_f = [];
    ...
    disp(x0_l)
    disp('following train position')
    disp(x0_f)
    
    % print results of current iteration
    plot_results([], xOpt_l, uOpt_l, xOpt_f, uOpt_f, predErr, x_pred_l, x_pred_f, p_sampled, DPspeed_, maxspeed_,47)
