function [feas, xOpt, uOpt, predErr, x_pred_l, x_pred_f, u_pred_l, u_pred_f] = MPC_leadingTrain_MovingBlock(x0_l, x0_f,param,MODEL,...
...
% DPspeed_     function handle for limspeed_ function
...
if blocksize>=param.Np
   disp('blocksize too big')
end
...
uOpt_l_t = 1*1.8449e+05*ones(1,param.Np) ;
...
while xOpt_l(1,end) < 1300 %p_sampled(1,end)
    tic
    % apriori estimation
...
%         slope_, radius_, limspeed_, maxspeed_);
   ...
   if feas_l_t ~= 1
       ...
       % run optimization for a second time, with one more 
...
%        else
            %feas_f = [feas_f, true];
            for j = 1:blocksize
                xOpt_l = [xOpt_l, xOpt_l_t(:,j+1)];
                %xOpt_f = [xOpt_f, xOpt_f_t(:,2)];
                uOpt_l = [uOpt_l, uOpt_l_t(1,j)];
                %uOpt_f = [uOpt_f, uOpt_f_t(1,1)];
            end
            x0_l = xOpt_l_t(:,blocksize+1) ;
            %x0_f = xOpt_f_t(:,2) ;
            %x_pred_f{i} = xOpt_f_t ;
 ...
 %      end
    ...
    plot_results([], xOpt_l, uOpt_l, [], [], predErr, x_pred_l, x_pred_f, p_sampled, DPspeed_, maxspeed_,22)
