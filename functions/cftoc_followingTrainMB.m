function [feas_f, xOpt_f, uOpt_f, JOpt_f] = cftoc_followingTrainMB(x0, ... 
     xbar, ubar, xleader, MODEL, param, slope_,radius_,DPspeed_,maxspeed_)
...
if MODEL == midterm
    ...
    for k = 1:Np+1 % sum from t to t+Np, because of matlab indexing -> from 1 to Np+1  
     %   d(1,k) = x(1,k) - xbar_l(1,k) - L;  % current distance to leader
        objective = objective + Kd * norm(xleader(1) - x(1,k) - L - d_des) + ...
    ...
    for i = 1:Np % constraints for time steps t to t+Np-1, have terminal constraints for timestep t+Np
        ...
        d_min <= xleader(1) - x(1,i) - L ...  % (11e)
        0 <= x(2,i+1) <= maxspeed_(xbar(1,i+1)) ... % (11f)
        % can't use (11g) without force in states
        -M*g*param.mumax*1e-4  <= u(1,i)*1e-4 <= M*g*param.mumax*1e-4]; % midterm equivalent of (11h)
    ...
    constraints = [constraints d_min <= xleader(1) - x(1,Np+1) - L ...  % (11l)
                    d_min <= xleader(1) - x(1,Np+1) - L  - (x(2,Np+1)^2/(2*a_f))];  % (11m)
    %% Optimize
    options = sdpsettings('verbose',0,'usex0',1,'solver','fmincon','fmincon.MaxIter',500000,...
        'fmincon.MaxFunEvals',5000000,'fmincon.TolFun',1e-06,'fmincon.TolFunValue',1e-06,'fmincon.TolCon',1e-03);
...
elseif MODEL == paper 
    ...
    d = zeros(Np+1);
    for k = 1:Np+1  % sum from t to t+Np, because of matlab indexing -> from 1 to Np+1  
        d(k) = x(1,k) - xleader - L;  % current distance to leader
    ...
    constraints = [constraints d_min <= d(Np+1) ...  % (11l)
                    d_min <= d(Np+1) - (x(2:Np+1)^2/(2*a_f))];  % (11m)
    %% Optimize
        options = sdpsettings('verbose',0,'usex0',1,'solver','fmincon','fmincon.MaxIter',500000,...
